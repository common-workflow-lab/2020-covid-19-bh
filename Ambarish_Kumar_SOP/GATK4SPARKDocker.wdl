...
task SortSamSpark {
 ...
 command {
      java -jar ${gatk_path} SortSamSpark -I ${inputBAM} -O ${sampleName}.bam }
runtime
{
  docker:docker
}
...
task MarkDuplicatesSpark {
 ...
 command {
    java -jar ${gatk_path} MarkDuplicatesSpark -I ${inputBAM} -O ${sampleName}.markdup.bam -M output.metrics
}
runtime
{
  docker:docker
...
task HaplotypeCallerSpark {
 ...
 command {
                ...
                java -jar ${gatk_path} HaplotypeCallerSpark -R ${Reference2bitGenome} -I ${inputBAM} -O ${sampleName}.mutant.vcf 
}
runtime
{
  docker:docker
